...
namespace NESEmulatorTests.CPU6502.InstructionSet.Operations.ControlOperations
{
    ...
    public class JumpTest
    {
        ...
        public void TestJump()
        {
            ...
            registers.StackPointer = 0x03;
            bus.CPUWrite(0x0102, 0x33);
            bus.CPUWrite(0x0101, 0x22);

            new Jump(false).OperationWithAddress(bus, registers, 0x950B);

            Assert.AreEqual(registers.ProgramCounter, 0x950B);
            Assert.AreEqual(registers.StackPointer, 0x03); //the stack did not change
        ...
        public void TestJumpWithProgramCounterPushOnStack()
        {
            ...
            Assert.AreEqual(registers.ProgramCounter, 0x950B);
            Assert.AreEqual(registers.StackPointer, 0x01); //the stack changed in order to push the program counter
