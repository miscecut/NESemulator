...
namespace NESEmulatorTests.CPU6502
{
    ...
    public class CPURegisterFlagsTest
    {
        ...
        public void TestSetFlags()
        {
            ...
            Assert.AreEqual(registers.GetStatus(), 0b00100000); //correct status init

            registers.SetFlag(StatusRegisterFlags.Carry, true);
            Assert.AreEqual(registers.GetStatus(), 0b00100001); //last significant bit is set;
            registers.SetFlag(StatusRegisterFlags.Overflow, true);
            Assert.AreEqual(registers.GetStatus(), 0b01100001); //sixth bit is set;
            registers.SetFlag(StatusRegisterFlags.Overflow, true);
            Assert.AreEqual(registers.GetStatus(), 0b01100001); //sixth bit is not changed
        }
        ...
        public void TestUnsetFlags()
        {
            ...
            Assert.AreEqual(registers.GetStatus(), 0b01100000); //Carry bit is unset
            Assert.IsFalse(registers.GetFlag(StatusRegisterFlags.Carry));
            registers.SetFlag(StatusRegisterFlags.Overflow, false);
            Assert.AreEqual(registers.GetStatus(), 0b00100000); //Carry bit is still unset
