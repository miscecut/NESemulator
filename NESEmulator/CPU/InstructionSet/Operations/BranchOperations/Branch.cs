...
namespace NESEmulator.CPU.InstructionSet.Operations.BranchOperations
{
    public class Branch : IOperation
    {
        ...
        public int OperationImmediate(IBus bus, CPURegisters registers) //this method shouldn't be called
        {
            return 0;
        }

        public int OperationWithAddress(IBus bus, CPURegisters registers, ushort address) //this is the effective operation, the address passed is the result of the Relative Address Mode
        {
            ...
            if(registers.GetFlag(_flagToCheckForBranch) == _branchIf)
            {
                ...
                var newProgramCounter = (ushort)(address + registers.ProgramCounter); //a new program counter is set, this is where the program will branch
                if (BytesUtils.GetHiByte(newProgramCounter) != BytesUtils.GetHiByte(registers.ProgramCounter))
                    additionalCycles++; //if a page is crossed, an additional cycle is required
                registers.ProgramCounter = newProgramCounter;
            }
